...
interface Question {
  step: number;
  text: string;
  educationalText: string; // Texte éducatif
  options: { value: string | number; label: string }[];
}

@Component({
  selector: 'app-educatif',
  templateUrl: './educatif.component.html',
  styleUrls: ['./educatif.component.css'],
  standalone: true,
  imports: [CommonModule, RouterModule, RecommendationComponent]
})
export class EducatifComponent implements OnInit {
  currentStep = 1;
  answers: Answers = {
    cpuModel: '',
    gpuModel: '',
  ...
  questions: Question[] = [
    {
      ...
      educationalText: 'Le CPU est le cerveau de l’ordinateur. Il gère toutes les tâches de calcul. Un CPU plus puissant (comme Intel X9 ou AMD B9) est idéal pour des tâches lourdes comme le gaming ou le montage vidéo.',
      options: [
    ...
    },
    {
      ...
      educationalText: 'Le GPU (carte graphique) est essentiel pour les jeux et les applications graphiques. Un GPU comme RTZ 3080 offre d’excellentes performances pour les jeux modernes, tandis que HD IRIS est mieux pour un usage basique.',
      options: [
    ...
    },
    {
      ...
      educationalText: 'La RAM est la mémoire vive. Plus vous en avez, plus votre ordinateur peut gérer de tâches simultanément. 8 Go est suffisant pour un usage basique, mais 16 Go ou plus est recommandé pour le gaming ou le multitâche.',
      options: [
        { value: 4, label: '4 Go' },
        { value: 8, label: '8 Go' },
    ...
    },
    {
      ...
      educationalText: 'Le stockage contient vos fichiers et programmes. Les SSD et NVMe sont beaucoup plus rapides que les HDD, ce qui rend votre ordinateur plus réactif. Les HDD sont moins chers mais plus lents.',
      options: [
    ...
    },
    {
      ...
      educationalText: 'Votre budget déterminera les options disponibles. Un budget plus élevé permet d’accéder à des ordinateurs plus puissants, mais il y a de bonnes options même à petit budget.',
      options: [
        { value: 500, label: '500 €' },
  ...
  generateRecommendation() {
    ...
    const filteredComputers = this.computers.filter(computer => {
      ...
      const matchesRam = !this.answers.ram || Math.abs(computer.system.ram - this.answers.ram) <= 8;
